...
OneBrain is an open-source AI thinking partner that runs inside your Obsidian vault: a two-way intelligence layer where you teach the agent and the agent extends you, both evolving over time against your own notes, memory, and tasks. It ships as a CLI (`onebrain-ai/onebrain`) with ~30 commands and a built-in web UI, with OneBrain Sync (cross-machine vault/memory sync, agent still local) planned.
...
OneBrain Sync notify-list signups are a secondary outcome. A precondition for all of these is *comprehension*: the site has to make "you teach the agent, it extends you, all inside your vault" land fast, because the core idea is unusual and easily mistaken for yet another note app.
